...
export function Wordmark({ className, href = "/" }: { className?: string; href?: string | null }) {
  ...
  const inner = (
    <span className={cn("inline-flex items-center gap-2.5 h-11 group", className)}>
      ...
      {/* Below 400px the shield carries the mark alone — the row also holds
           a language picker, an avatar, a CTA and a menu button, and the link
           keeps its "Kavach, home" label either way. */}
      <span className="max-[399px]:hidden font-display text-[1.375rem] leading-none tracking-tight">
        Kavach
      </span>
    </span>
  );
